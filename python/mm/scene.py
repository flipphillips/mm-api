...
def append_vertex(remote, objectid, position, normal=(0, 1, 0), color=(1, 1, 1)):
    ...
    cmd1 = mmapi.StoredCommands()
    key1 = cmd1.AppendSceneCommand_AppendVertex(
        objectid, to_vec3f(position), to_vec3f(normal), to_vec3f(color))
    remote.runCommand(cmd1)
    result_val = mmapi.any_result()
...
def append_triangle(remote, objectid, newtri, newgroup=0):
    ...
    cmd1 = mmapi.StoredCommands()
    t = to_vec3i(newtri)
    key1 = cmd1.AppendSceneCommand_AppendTriangle(
        objectid, to_vec3i(newtri), newgroup)
    remote.runCommand(cmd1)
    result_val = mmapi.any_result()
...
def set_group(remote, objectid, triangleid, groupid):
    cmd1 = mmapi.StoredCommands()
    key1 = cmd1.AppendSceneCommand_SetTriangleGroup(
        objectid, triangleid, groupid)
    remote.runCommand(cmd1)
